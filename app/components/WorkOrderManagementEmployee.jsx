...
import { Wrench, X, Car, Plus, FileText } from "lucide-react";
...
const WorkOrderManagementEmployee = () => {
    ...
    const getWorkOrders = async () => {
        try {
            const response = await axios.get("/api/employee/getWorkOrders");
    ...
    const createWorkOrderMutation = useMutation({
        mutationFn: async (data) => {
            const response = await axios.post("/api/employee/createWorkOrder", data);
        ...
        <div className="flex min-h-screen bg-gray-50/50">
            ...
            <main className="flex-1 p-8 lg:p-10 max-w-7xl mx-auto w-full">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-gray-200 pb-6 mb-8">
                    <div>
                        <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">My Work Orders</h1>
                        <p className="text-sm text-gray-500 mt-2">Log and view your vehicle maintenance requests and workshop services.</p>
                    </div>
                    <button
                ...
                {workOrders && workOrders.length === 0 && !workOrdersLoading && (
                    <div className="text-center py-12 bg-white border border-gray-200 rounded-2xl shadow-sm">
                        ...
                        <p className="text-gray-500 text-sm font-medium">You haven't logged any work orders yet.</p>
                    </div>
                )}
                                        ...
                                        <tr key={wo._id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-gray-400 mt-0.5 max-w-[250px] truncate" title={wo.comments}>{wo.comments || "No comments"}</p>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div>
...
export default WorkOrderManagementEmployee;
